...
class ScopesTest : BaseTest {
    ...
    fun nestedInjection() {
        ...
        val injectedFile = JavaFileObjects.forSourceLines("test.MainActivityInjector",
            ...
            "       target.browserUi = provideBrowserUi(target);",
            "       target.session = provideSession(target);",
            "       target.logger = provideLogger(target);",
            "   }",
            "",
            "   private final BrowserUi provideBrowserUi(@NonNull final MainActivity target) {",
            "       BrowserUi browserUi2 = new PhoneBrowserUi(target);",
            "       return browserUi2;",
            "   }",
            "",
            "   private final Session provideSession(@NonNull final MainActivity target) {",
            "       Session session = new SessionImplementation(target.browserUi, target.getSecondScoped());",
            "       return session;",
            "   }",
            "",
            "   private final Logger provideLogger(@NonNull final MainActivity target) {",
    ...
    fun localScopeInConstructor() {
        ...
        val injectedFile = JavaFileObjects.forSourceLines("test.AddressBarListenerImplInjector",
            ...
            "   }",
            "",
            "   private final AutocompleteController provideAutocompleteController(@NonNull final AddressBarListenerImpl target) {",
