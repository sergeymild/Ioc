...
class ScopesTest : BaseTest {
    ...
    fun nestedInjection() {
        ...
        val injectedFile = JavaFileObjects.forSourceLines("test.MainActivityInjector",
            ...
            "       target.setSecondScoped(injectSecondScopedInSecondScoped());",
            "       target.browserUi = injectBrowserUiInBrowserUi(target);",
            "       target.session = injectSessionInSession(target);",
            "       target.logger = injectLoggerInLogger(target);",
            "   }",
            "",
            "   private final SecondScoped injectSecondScopedInSecondScoped() {",
            "       SecondScoped secondScoped = new SecondScoped();",
            "       return secondScoped;",
            "   }",
            "",
            "   private final BrowserUi injectBrowserUiInBrowserUi(@NonNull final MainActivity target) {",
            "       BrowserUi browserUi2 = new PhoneBrowserUi(target);",
            "       return browserUi2;",
            "   }",
            "",
            "   private final Session injectSessionInSession(@NonNull final MainActivity target) {",
            "       Session session = new SessionImplementation(target.browserUi, target.getSecondScoped());",
            "       return session;",
            "   }",
            "",
            "   private final Logger injectLoggerInLogger(@NonNull final MainActivity target) {",
    ...
    fun localScopeInConstructor() {
        ...
        val injectedFile = JavaFileObjects.forSourceLines("test.AddressBarListenerImplInjector",
            ...
            "       target.controller = injectAutocompleteControllerInController(target);",
            "   }",
            "",
            "   private final AutocompleteController injectAutocompleteControllerInController(@NonNull final AddressBarListenerImpl target) {",
